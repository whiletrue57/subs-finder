...
def write_outputs(selected: list[dict], validated_count: int, args) -> None:
    ...
    # 收集 TG 节点数（如果 tg-nodes.yaml 存在且非空）
    tg_yaml = out_dir / "tg-nodes.yaml"
    tg_count = 0
    if tg_yaml.exists():
        try:
            data = yaml.safe_load(tg_yaml.read_text(encoding="utf-8"))
            if isinstance(data, dict) and isinstance(data.get("proxies"), list):
                tg_count = len(data["proxies"])
        except Exception:
            pass
    ...
    lines = [
        ...
        f"# generated_at: {now}",
        f"# top: {len(top)}  validated: {validated_count}  tg_nodes: {tg_count}",
        "# do not edit by hand; see subs-finder/find_clash.py",
        "",
    ]
    lines.extend(e["url"] for e in top)
    # 如果 TG 节点文件存在且有内容，追加其 raw URL 供 subs-check 拉取
    if tg_count > 0:
        tg_raw_url = (
            "https://raw.githubusercontent.com/"
            + os.environ.get("GITHUB_REPOSITORY", "")
            + "/main/output/tg-nodes.yaml"
        )
        if os.environ.get("GITHUB_REPOSITORY"):
            lines.append(tg_raw_url)
    lines.append("")
    ...
    print(f"[done] wrote {txt_path} ({len(top)} urls, tg_nodes={tg_count})", file=sys.stderr)
